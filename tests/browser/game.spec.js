...
test('launch, fly, discover a beacon, pause, and restart without rendering errors', async ({ page }) => {
  const errors = [];
  ...
  await expect.poll(() => page.locator('audio[data-sound="bg-music"]').evaluate((audio) => !audio.paused && audio.currentTime > 0)).toBe(true);
  await expect.poll(() => page.locator('audio[data-sound="engine"]').evaluate((audio) => !audio.paused && audio.volume > 0)).toBe(true);
  await page.getByRole('button', { name: 'Mute audio', exact: true }).click();
  expect(await page.locator('audio').evaluateAll((tracks) => tracks.every((audio) => audio.muted))).toBe(true);
  await page.getByRole('button', { name: 'Unmute audio', exact: true }).click();
  await page.keyboard.down('Shift');
  await expect.poll(async () => Number(await page.locator('#speed').textContent())).toBeGreaterThan(180);
  await expect(page.locator('#discovered-count')).toHaveText('01', { timeout: 15000 });
  await expect.poll(() => page.locator('audio[data-sound="found"]').evaluate((audio) => audio.currentTime > 0)).toBe(true);
  ...
  expect(await page.locator('audio').evaluateAll((tracks) => tracks.every((audio) => audio.paused))).toBe(true);
...
test('controls open before launch and touch controls work at a mobile viewport', async ({ browser }) => {
  ...
  await expect.poll(async () => Number((await page.locator('#thrust-value').textContent()).replace('%', ''))).toBeGreaterThan(40);
  await page.mouse.up();
