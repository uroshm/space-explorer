...
export function createShip() {
  ...
  for (const side of [-1, 1]) {
    ...
    const engine = mesh(new THREE.CylinderGeometry(0.46, 0.52, 3.2, 12), dark, [
      side * 1.65,
      0,
      1.55,
    ]);
    engine.rotation.x = Math.PI / 2;
    const nozzle = mesh(new THREE.CylinderGeometry(0.33, 0.33, 0.12, 16), light, [
      side * 1.65,
      0,
      3.2,
    ]);
  ...
  for (const side of [-1, 1]) {
    const plume = mesh(
      new THREE.ConeGeometry(0.32, 3.6, 16, 1, true),
