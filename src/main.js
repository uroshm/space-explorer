...
function startGame() {
  ...
  function updateApproachAlert() {
    ...
    const activeCandidate = candidates.find(
      ({ planet, surfaceDistance }) => planet.id === activeApproachId && surfaceDistance <= 5800,
    );
    const approaching =
      activeCandidate ??
      candidates
    ...
    if (!paused) {
      ...
      flight.update(dt, {
        ...
        yaw: THREE.MathUtils.clamp(
          pressed('KeyD', 'ArrowRight') - pressed('KeyA', 'ArrowLeft') + mouse.x,
          -1,
          1,
        ),
        ...
        if (!destination.discovered && flight.position.distanceTo(destination.position) < 100) {
          ...
          notify(
            count === world.destinations.length
              ? `System surveyed. All ${count} places discovered — keep exploring.`
              : `Discovered: ${destination.name}. ${destination.info}`,
          );
          if (
